...
const AddProductsUI = ({setProductName,setShorDesc,setCategory,handleColorModal,selectedColors,womenSizeRangesCM,setSelectChestSize,selectChestSize,
    ...
    return (
      <View>
        ...
        <Text className="text-[#fff] font-instrumentSansSemiBold mt-2 mb-2">Colors*</Text>
        <TouchableOpacity className="flex-row items-center mt-2 gap-4 border rounded-xl bg-[#2C2C2C] px-1 " onPress={handleColorModal}>
          <View className="flex-1 gap-2 mt-2 mb-2 bg-[#2C2C2C] p-3 rounded-lg ">
            <Text className="text-white ">{selectedColors.map((item: any) => item.name).join(", ")}</Text>
          </View>
        </TouchableOpacity>
          ...
          <View className="flex-row items-center">
            {isAvailble ? (
              <TouchableOpacity onPress={() => setIsAvailable(false)}>
                <MaterialCommunityIcons name="toggle-switch" size={54} color="white" />
              </TouchableOpacity>
            ) : (
              <TouchableOpacity onPress={() => setIsAvailable(true)}>
                <MaterialCommunityIcons name="toggle-switch-off" size={54} color="white" />
              </TouchableOpacity>
            )}
          </View>
        </View>
