          ...
          <ScrollView contentContainerStyle={{ padding: 15, paddingBottom: 100 }}>
            <View className="mt-2 mb-2 flex-row bg-[#212121] rounded-lg overflow-hidden p-3 gap-2 items-center">
              ...
              <View className="flex-1 flex-row justify-between  items-center">
                ...
                <View className="items-center justify-center" style={{ width: scale(48), height: verticalScale(48) }}>
                  <Image source={require("../../../assets/e-icon/cameraWhite.png")} style={{ width: "50%", height: "50%" }} />
                </View>
              </View>
            </View>
            <View className="flex-row gap-3 mt-1 mb-2">
              <TouchableOpacity
                className={`${isClothType == "ALL Results" ? "bg-[#DCF3FF]" : "bg-[#1D3725]"} rounded-md items-center p-1 `}
                onPress={() => setIsClothType("ALL Results")}
              >
                <Text className={`font-instrumentSansBold ${isClothType == "ALL Results" ? "text-[#121212]" : "text-white"}`}>
                  ALL Results
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
                className={`${isClothType == "In Stock" ? "bg-[#DCF3FF]" : "bg-[#1D3725]"} rounded-md items-center p-1`}
                onPress={() => setIsClothType("In Stock")}
              >
            ...
            <ScrollView horizontal>
              <View className="flex-row gap-2">
                {getSearchData?.data?.product?.length > 0 ? (
                  getSearchData?.data?.product?.map((item, index) => (
                    <TouchableOpacity
                      key={index}
                    ...
                    >
                      ...
                      <View className="p-2">
                        ...
                        <Text className="text-[#9CA3AF] mb-1 font-instrumentRegular">{item.brandName}</Text>
                  ...
                  <Text className="text-white font-instrumentSansSemiBold text-center mt-4">No products found</Text>
                )}
              </View>
            </ScrollView>
