...
import { useGetBrandOrderListQuery, usePostStatusOrderBrandMutation } from 'src/redux/features/orders/orderApi'
...
const OrderList = () => {
    ...
    const handleStatus = async (id: any, status: any) => {
     ...
     if(getOrdersBrand?.data?.data?.length===0){
    ...
    return (
        <View className='flex-1 p-3'>
            <ScrollView showsVerticalScrollIndicator={false}>
                {getOrdersBrand?.data?.data?.map((item: any) => <View key={item.cartProductId} className='bg-[#212121] p-2 rounded-xl mt-1 mb-2' >
