        ...
        <View className='flex-1 bg-[#121212] p-5 relative'>
            ...
            <View className="mt-1 mb-2">
                <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 10 }} style={{ height: 40 }}>
                    {feedCatgory?.map(item => <TouchableOpacity key={item}
                        className={`${selectedItem == item ? "bg-[#DCF3FF]" : "bg-[#1D3725]"} rounded-full items-center justify-center px-4 mr-2 `}
                        onPress={() => handleSaveCat(item)}
                    >
                        <Text className={`font-instrumentSansSemiBold ${selectedItem == item ? "text-[#121212]" : "text-white"}`}>{item}</Text>
                    </TouchableOpacity>)}
                </ScrollView>
            </View>
