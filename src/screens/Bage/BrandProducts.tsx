...
console.log("isError:", isError)
    ...
    const filteredProducts = useMemo(() => {
        let products: AllProduct[] = data?.data?.product ?? [];

        if (isClothType !== "ALL") {
            products = products.filter((item) => item.category === isClothType);
        }
        ...
        return products;
    }, [data, isClothType, searchQuery]);

    const renderProduct = (item: AllProduct, index: number) => (
        <TouchableOpacity
            key={index}
            style={{ width: "48%" }}
    ...
    return (
        <View className='flex-1 bg-[#121212] p-3'>
            ...
            <View className='border p-2 mt-2 mb-2 rounded-xl bg-[#252525] flex-row gap-3 items-center'>
                ...
                <TextInput className='flex-1 font-instrumentSansSemiBold text-white' placeholder='Search Brands...' placeholderTextColor={"#ADAEBC"} value={searchQuery}
                    onChangeText={setSearchQuery} />
            </View>

            <View className='flex-row gap-3 mt-1 mb-2'>
                {catArr?.map((item, index) => <TouchableOpacity key={index} className={`${isClothType == item ? "bg-[#DCF3FF]" : "bg-[#1D3725]"} rounded-md items-center p-1 `} onPress={() => setIsClothType(item)}>
            ...
            <ScrollView contentContainerStyle={{ paddingBottom: 100 }} showsVerticalScrollIndicator={false}>
                ...
                <View className='flex-row flex-wrap  justify-between gap-2'>
                    {filteredProducts.length > 0 ? (
                        filteredProducts.map((item, index) => renderProduct(item, index))
                    ) : (
