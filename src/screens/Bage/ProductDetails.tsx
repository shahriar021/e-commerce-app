        ...
        <View className='flex-1 bg-[#121212] p-3'>
            <ScrollView contentContainerStyle={{ alignItems: "center", paddingBottom: 100 }}>
                ...
                <View className='w-full  mt-4 mb-3 p-3 flex-row justify-between items-center'>
                    <View className='flex-col'><Text className='text-white font-instrumentSansBold'>{data?.data?.product[0]?.productName}</Text><Text className='text-white font-instrumentSansSemiBold'>${data?.data?.product[0]?.price}</Text></View>
                    <View className='flex-row items-center gap-2'>
                        <TouchableOpacity className='bg-[#252525] p-1 rounded-full' onPress={() => handleQuantity("minus")}>
                            <AntDesign name="minus" size={24} color="white" />
                        </TouchableOpacity>
                        <Text className='text-white'>{quantity}</Text>
                        <TouchableOpacity className='bg-[#252525] p-1 rounded-full' onPress={() => handleQuantity("add")}>
                ...
                <View className='w-full p-3 '>
                    ...
                    <View className='flex-row justify-between mt-2 mb-1'>
                        <View className='flex-row gap-2 items-center'>
                            <View style={{ width: scale(30), height: scale(30) }}>
                                <Image source={{ uri: getReview?.data?.data[0]?.userInfo?.profile[0] }} style={{ width: "100%", height: "100%" }} />
                            </View>
                            <View className='flex-col  gap-2'>
                                <View className='flex-row gap-2 items-center'>
                                    <Text className='text-white font-instrumentSansSemiBold'>{getReview?.data?.data[0]?.userInfo?.userName}</Text>
                                ...
                                <Text className='text-[#ADAEBC] font-instrumentRegular'>{getTime(getReview?.data?.data[0]?.createdAt)}</Text>
                            </View>
                    ...
                    <Text className='font-instrumentRegular text-[#fff] mt-2'>{getReview?.data?.data[0]?.comments}</Text>
                    <View className='mt-2 rounded-xl overflow-hidden' style={{ width: scale(111), height: verticalScale(111) }}>
                        <Image source={{ uri: getReview?.data?.data[0]?.attachment[0] }} style={{ width: "100%", height: "100%" }} />
                    </View>

                    <TouchableOpacity className='bg-[#1D3725] flex-row items-center justify-center gap-2 mt-3 p-3 rounded-xl' onPress={handleAddToCart}>
