...
const Bage = ({navigation}:Props) => {
  ...
  const { data, isLoading: isQueryLoading,isFetching, refetch } = useFeatureBrandsQuery({ token, limit: loadMore }) 
  ...
  const displayData = useMemo(() => {
    ...
    // Sort Safely (Prevent crash if brandName is missing)
    ...
    <View className='flex-1 bg bg-[#121212] p-4 items-center'>
      ...
      {isQueryLoading ? (
        ...
        >
          ...
          {displayData.length > 0 && (
            <TouchableOpacity 
              className='bg-[#1D3725] p-2 items-center mt-4 mb-4 rounded-xl overflow-hidden w-full' 
              onPress={() => setLoadMore(prev => prev + 10)}
            >
